...
class ChannelLogItem:
    
    def to_string(self):
        return "<%s> %s" % (self.user.split('!')[0], self.text)
    
    def __init__(self, channel, user, text, datetime = datetime.now()):
...
class ChannelLog:
    ...
    def append(self, log_item):
        self.LogItems.append(log_item)
        self.__sort()
        self.__truncate()
        
    def exchange_set(self, channel, user, lines_back, num_lines):
        if lines_back - 1 - num_lines < 0 or lines_back < 0:
            raise IndexError
        if lines_back == 0:
            lines_back = 1
        exchange_items = filter(
            #lambda (index, logitem): logitem.channel == channel and index < lines_back - 1 and index > lines_back - num_lines - 1
            lambda logitem: logitem in self.LogItems[lines_back - 1 - num_lines:lines_back - 1]
            ,self.LogItems
        )
        
        return exchange_items
    
    def last_said(self, channel, nick):
        #print [logitem.to_string() for logitem in self.LogItems]
        LastLogItems = filter(
            lambda logitem: logitem.channel == channel and logitem.user.split('!')[0] == nick
            ,self.LogItems
            )
        #print LastLogItems
        return LastLogItems[0]
    
    def next(self):
        if self.count >= len(self.LogItems):
            self.count = 0
            raise StopIteration
        else:
            c = self.count
            self.count = c + 1
            return self.LogItems[c]
    
    def __init__(self, line_limit = 500):
        self.LogItems = []
        self.count = 0
        self.line_limit = line_limit
    
    def __sort(self):
        self.LogItems.sort(key=operator.attrgetter('datetime'),reverse=True)

    def __truncate(self):
        self.LogItems = self.LogItems[:self.line_limit]
    
    def __getitem__(self, item):
        return self.LogItems[item]
...
class PyFiBot(irc.IRCClient, CoreCommands):
    ...
    def __init__(self, network, cmdchar = '.'):
        if cmdchar != 'nochar':
    ...
    def timerEvent(self, delay):
        self.timer_counter = self.timer_counter + delay
        self._runhandler('timerEvent', self.timer_counter)
        reactor.callLater(delay, self.timerEvent, delay)
    ...
    def userRenamed(self, oldnick, newnick):
        ...
        self.track_remove(oldnick, channel)
        self.track_add(newnick, channel)
